...
export default function UpdatesContent({ updates }: { updates: UpdateEntry[] }) {
  ...
  return (
    <>
      ...
      >
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: "var(--text-lg)", fontWeight: 600, letterSpacing: "-0.01em", margin: 0 }}>
          Updates
        </h1>
      </header>
      ...
      <div style={{ padding: "20px 20px", paddingBottom: "calc(var(--nav-clearance) + 16px)", maxWidth: "40rem", margin: "0 auto" }}>
        {updates.map((u) => (
          ...
          >
            ...
            {/* Body */}
            <p style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.6, margin: 0, marginTop: 8 }}>
              {u.body}
            </p>
            ...
            {u.song_names.length > 0 && (
              <div style={{ marginTop: 8 }}>
                {u.song_names.map((name) => (
                  <div key={name} style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.6 }}>
                    &rsaquo; {name}
                  </div>
                ))}
              </div>
